...
module Wechat
  module Controller::Application
    ...
    def current_oauth_app
      ...
      if request.variant.include?(:work_wechat)
        @current_oauth_app = current_organ&.agents&.take || Agent.first
      else
    ...
    def current_wechat_user
      ...
      if @current_wechat_user
        if request.variant.include?(:mini_program) && current_user
          if @current_wechat_user.is_a?(ProgramUser)
            logger.debug "\e[35m  Current Program User: #{@current_wechat_user.id}  \e[0m"
            return @current_wechat_user
          end
          ...
          if @current_wechat_user.is_a?(WechatUser)
    ...
    def login_by_oauth_user(oauth_user, url: url_for(controller: '/home'))
      ...
      if state
        ...
        url = url_for(
          controller: '/home',
          host: Current.session.auth_app.host,
          auth_jwt_token: Current.session.generate_jwt_token,
          state: params[:state]
        )
    ...
    def login_by_corp_user(corp_user, url: url_for(controller: '/home'))
      ...
      if state
        state.update auth_token: Current.session.once_token, destroyable: true
        render 'state_visit', layout: 'raw', locals: { state: state }
      else
